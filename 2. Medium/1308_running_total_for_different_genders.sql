...
-- My Answer 
WITH GroupScore AS 
(SELECT S.gender, S.day, SUM(score_points) score_point
FROM Scores S 
...
-- What I learned
1) You could use OVER(PARTITION BY   - ORDER BY ) to get accumulative sums 
2) ORDER BY helps apply accumulated aggregations 
...
WITH AccumTable AS 
(SELECT experience, employee_id, salary, SUM(salary) OVER (PARTITION BY experience ORDER BY salary ASC) accum_salary
FROM Candidates C
ORDER BY experience, salary)

SELECT employee_id
FROM AccumTable AT 
WHERE accum_salary <= 70000
AND experience = "Senior"
UNION 
SELECT AT.employee_id 
FROM (SELECT 70000-(SELECT MAX(CASE WHEN accum_salary <= 70000
AND experience = "Senior" THEN accum_salary ELSE 0 END) 
FROM AccumTable AT) salary_left) LS, AccumTable AT 
WHERE AT.accum_salary <= LS.salary_left

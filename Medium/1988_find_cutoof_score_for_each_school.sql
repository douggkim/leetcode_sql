...
-- My Answer 
WITH ScoreMapTable AS 
(SELECT S.school_id, IFNULL(E.score, -1) AS score
FROM Schools S 
LEFT OUTER JOIN Exam E 
ON S.capacity >= E.student_count) 

SELECT  SMT.school_id, IFNULL(MIN(SMT.score), -1) AS score  
FROM ScoreMapTable AS SMT
GROUP BY SMT.school_id
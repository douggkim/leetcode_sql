...
-- My Answer 
WITH student_summary_table AS 
(SELECT S.dept_id, COUNT(*) student_cnt
FROM Student S 
GROUP BY S.dept_id)

SELECT D.dept_name dept_name, IFNULL(SS.student_cnt, 0) AS student_number
FROM Department D 
LEFT OUTER JOIN  student_summary_table SS
